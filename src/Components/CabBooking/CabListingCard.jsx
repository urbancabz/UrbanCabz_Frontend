...
export default function CabListingCard({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm hover:shadow-2xl hover:-translate-y-1 border border-slate-100 dark:border-slate-800 transition-all duration-300 overflow-hidden group">
      <div className="p-5 sm:p-7">
        {/* Mobile: Stack vertically, Desktop: Horizontal layout */}
        <div className="flex flex-col sm:flex-row gap-5 sm:gap-8">
          {/* CAR IMAGE */}
          <div className="w-full sm:w-40 flex-shrink-0 mx-auto sm:mx-0">
            <div className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 rounded-2xl h-36 sm:h-32 flex items-center justify-center overflow-hidden p-4 border border-slate-200 dark:border-slate-700 group-hover:border-yellow-400/30 transition-colors">
              {image ? (
                <img
                  src={image}
                  alt={name}
            ...
            <div className="text-center text-xs mt-3 text-slate-500 dark:text-gray-400 font-bold uppercase tracking-wider bg-slate-100/50 dark:bg-slate-800/50 rounded-full py-1.5 px-3">
              {vehicleType}
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0 flex flex-col justify-between">
            <div>
              {/* Header: Name, Rating */}
              <div className="mb-4">
                <h3 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white mb-2 truncate">{name}</h3>
                {rating && (
                  <div className="text-sm text-yellow-600 dark:text-yellow-400 flex items-center gap-1.5 bg-yellow-50 dark:bg-yellow-900/20 w-fit px-2 py-0.5 rounded-lg border border-yellow-100 dark:border-yellow-900/30">
                    <svg className="w-4 h-4 fill-current" viewBox="0 0 20 20">
                      <path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z" />
                    </svg>
                    <span className="font-bold">{rating}</span>
                    <span className="text-yellow-400 mx-1">|</span>
                    <span className="text-yellow-700/80 dark:text-yellow-200/80 font-medium">Top Rated</span>
                  </div>
                )}
              </div>

              {/* Features: Seats, Bags, Distance */}
              <div className="flex flex-wrap gap-3 sm:gap-6 items-center text-sm text-slate-600 dark:text-gray-400 mb-4">
                ...
                {distanceKm && (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-lg border border-blue-100 dark:border-blue-900/30">
              ...
              <div className="flex flex-wrap gap-2">
                {tags.map((t, i) => (
                  <span
                    key={i}
                    className="text-[10px] uppercase tracking-wider px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-gray-400 rounded-md font-bold border border-slate-200 dark:border-slate-700"
                  >
                    {t}
                  </span>
                ))}
          ...
          <div className="flex flex-col items-end justify-between sm:border-l border-slate-100 dark:border-slate-800 sm:pl-8 sm:min-w-[180px]">
            {/* Price Section */}
            <div className="text-right w-full sm:w-auto mb-4 sm:mb-0">
              ...
              <div className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white leading-none">
                {price > 0 ? `₹${price.toLocaleString()}` : <span className="text-slate-300 dark:text-gray-600 text-xl">---</span>}
              </div>
              {price > 0 && (
